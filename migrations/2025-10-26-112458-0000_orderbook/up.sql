...
create unique index idx_unique_order_per_wallet_market on OrderBook(wallet, market_id, status) where status = 'open';
create unique index idx_unique_order_per_wallet_assets on OrderBook(wallet, bid_asset, ask_asset, status) where status = 'open';
create unique index idx_unique_order_per_market_assets on OrderBook(market_id, bid_asset, ask_asset, status) where status = 'open';
create unique index idx_unique_order_per_wallet_market_assets on OrderBook(wallet, market_id, bid_asset, ask_asset, status) where status = 'open';
create unique index idx_unique_order_per_wallet_market_mode on OrderBook(wallet, market_id, mode, status) where status = 'open';
create unique index idx_unique_order_per_wallet_assets_mode on OrderBook(wallet, bid_asset, ask_asset, mode, status) where status = 'open';